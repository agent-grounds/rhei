...
fn validate_force_options(reason: Option<&str>, supervisor: Option<&str>, no_callbacks: bool) -> MietteResult<()> {
    if supervisor.is_some() { return Err(miette!("--force cannot be combined with --supervisor")); }
    if no_callbacks { return Err(miette!("a forced recovery runs no callbacks; drop --no-callbacks")); }
    if reason.is_none_or(|reason| reason.trim().is_empty()) {
        return Err(miette!("--force requires a fresh non-empty --reason"));
    }
    Ok(())
}
...
fn confirm_operator_hop(expected: &str, operation: &str) -> MietteResult<String> {
    ...
    if !std::io::stdin().is_terminal() {
        return Err(miette!("{operation} requires an interactive terminal and fresh typed confirmation"));
    }
    ...
    let account = std::process::Command::new("whoami").output()
        .map_err(|err| miette!("cannot identify invoking OS account: {err}"))?;
    let account_name = String::from_utf8(account.stdout)
        .map_err(|err| miette!("cannot decode invoking OS account: {err}"))?;
    if !account.status.success() || account_name.trim().is_empty() {
        return Err(miette!("cannot identify invoking OS account"));
    }
    eprintln!("Operator {}: type {expected}", account_name.trim());
    std::io::stderr().flush().map_err(|err| miette!("cannot print operator confirmation: {err}"))?;
    let mut response = String::new();
    let read = std::io::stdin().read_line(&mut response)
        .map_err(|err| miette!("cannot read operator confirmation: {err}"))?;
    if read == 0 || response.trim_end_matches(['\r', '\n']) != expected {
        return Err(miette!("operator confirmation did not match the exact hop; no changes made"));
    }
...
fn operator_run_locks(roots: &[PathBuf], owner: &str) -> MietteResult<Vec<HeldRunLock>> {
    ...
    for root in roots {
        let Some(mut lock) = try_acquire_run_lock(root)? else {
            ...
            return Err(miette!("execution root {} has a held run lock; recorded owner: {}", root.display(), recorded.trim()));
        };
        ...
        lock.file.rewind().and_then(|()| lock.file.set_len(0))
            ...
            .map_err(|err| miette!("cannot record operator run-lock owner: {err}"))?;
        held.push(lock);
    }
    Ok(held)
}
