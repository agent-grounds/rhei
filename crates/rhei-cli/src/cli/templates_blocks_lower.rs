    ...
    fn instantiate_compiled_workspace(
        ...
        let cwd = std::env::current_dir().map_err(|e| miette!("current directory: {e}"))?;
        ...
        let scratch = if dry_run { Some(tempfile::tempdir().map_err(|e| miette!("compiler output scratch: {e}"))?) } else { None };
        ...
        let files = compiled.files().map_err(|e| miette!("{e}"))?;
