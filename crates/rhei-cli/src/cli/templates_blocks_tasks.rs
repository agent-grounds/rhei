    ...
    impl BlockFrontend {
        fn render_fragment(&mut self, dir: &Path, reference: &str, values: &BTreeMap<String, serde_json::Value>, has_children: bool) -> MietteResult<Fragment> {
            ...
            let machine = if rendered.join("states.yaml").is_file() {
                ...
                return Err(miette!("{}: a local mounted plan must supply its states.yaml fragment", source.display()));
            } else {
            ...
            let (plan, tasks) = match layout {
                TemplateLayout::SingleFile => {
                    let text = files.remove(Path::new("plan.rhei.md")).ok_or_else(|| miette!("missing plan.rhei.md"))?;
                    let text = String::from_utf8(text.bytes).map_err(|e| miette!("{}/plan.rhei.md: {e:?}", dir.display()))?;
                    let mut plan = rhei_core::parse(&text).map_err(|e| miette!("{}/plan.rhei.md: {e:?}", dir.display()))?;
                    let tasks = vec![TaskFile { path: PathBuf::from("tasks/01-plan.md"), tasks: std::mem::take(&mut plan.tasks) }];
                    (plan, tasks)
                }
                TemplateLayout::Workspace => {
                    let text = files.remove(Path::new("index.rhei.md")).ok_or_else(|| miette!("missing index.rhei.md"))?;
                    let text = String::from_utf8(text.bytes).map_err(|e| miette!("{}/index.rhei.md: {e:?}", dir.display()))?;
                    let index = rhei_core::parser::parse_workspace_index(&text).map_err(|e| miette!("{}/index.rhei.md: {e:?}", dir.display()))?;
                    let paths = files.keys().filter(|p| p.starts_with("tasks") && p.extension().is_some_and(|e| e == "md")).cloned().collect::<Vec<_>>();
                    let mut tasks = Vec::new();
                    for path in paths {
                        let text = String::from_utf8(files.remove(&path).unwrap().bytes).map_err(|e| miette!("{}: {e:?}", dir.join(&path).display()))?;
                        let nodes = rhei_core::parser::parse_workspace_tasks_with_structure(&text, &index.structure).map_err(|e| miette!("{}: {e:?}", dir.join(&path).display()))?;
    ...
    fn read_compiled_files(root: &Path, dir: &Path, files: &mut BTreeMap<PathBuf, CompiledFile>) -> MietteResult<()> {
        ...
        for entry in entries {
            let path = entry.map_err(|e| miette!("read block entry: {e:?}"))?.path();
