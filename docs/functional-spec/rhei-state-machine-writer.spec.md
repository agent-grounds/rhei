...
- **A Panta project's custom default:** `states.yaml` at the project root (the directory holding `index.panta.md`), named by that manifest's `**States:**`. Every rhei that declares no `**States:**` runs under it. When an adopted project has no matching project-root file, one unique matching member-root file may supply this custom default.
...
- **One rhei's own process:** `states.yaml` at that rhei's execution root, named by the rhei's own `**States:**`. Every instantiated template ships this shape. An explicit declaration of a *custom* default's name gives that matching local file precedence even when it restates the project default's name; omit `**States:**` when the rhei should inherit the already-resolved project machine wholesale instead. Restating the project's effective built-in `rhei` default is different and stays equivalent to omission in every respect: the rhei's own root is not consulted there either, so it can supply a custom default only through that default's unique name-match fallback and can never supply the built-in `rhei` project default from a member root ([§DA-per-rhei-state-machines](../decisions/architectural/per-rhei-state-machines.md#da-per-rhei-state-machines-the-state-machine-is-a-per-rhei-property-defaulted-by-the-manifest)).
