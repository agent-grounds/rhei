...
id, so neither a live run nor any other project command can discover partial
output. The atomic rename to the requested member path publishes the complete,
project-valid member exactly once. A live unrestricted `rhei run` may admit it
at its next checkpoint ([§FS-rhei-run.3](rhei-run.spec.md#3-execution-loop)); an explicit
`--rhei` selection remains fixed. A program handing follow-on work to its
enclosing run invokes `rhei instantiate` without `--execute`. The
`--execute` flag retains its separate-run meaning and is not a nested-run
handoff mechanism.
